    ...
    */
    get: function (bucket, key, cb) {
        ...
        this.db.collection(this.prefix + this.removeUnsupportedChar(collName), (err, collection) => {
            ...
            collection.findOne(searchParams, { projection: {_bucketname: 0 }}, (err, doc) => {
    ...
    add: function (transaction, bucket, key, values) {
        ...
        transaction.push((cb) => {
            ...
            self.db.collection(self.prefix + self.removeUnsupportedChar(collName), function (err, collection) {
                ...
                // update document
                collection.updateMany(updateParams, { $set: doc }, { safe: true, upsert: true }, function (err) {
                    if (err instanceof Error) return cb(err);
                    cb(undefined);
                });
            });
    ...
    */
    del: function (transaction, bucket, keys) {
        ...
        transaction.push((cb) => {
            self.db.collection(self.prefix + self.removeUnsupportedChar(collName), function (err, collection) {
                if (err instanceof Error) return cb(err);
                collection.removeMany(updateParams, { safe: true }, (err) => {
                    if (err instanceof Error) return cb(err);
                    cb(undefined);
                });
            });
    ...
    remove: function (transaction, bucket, key, values) {
        ...
        transaction.push((cb) => {
            self.db.collection(self.prefix + self.removeUnsupportedChar(collName), function (err, collection) {
                ...
                // update document
                collection.updateMany(updateParams, { $unset: doc }, { safe: true, upsert: true }, function (err) {
                    if (err instanceof Error) return cb(err);
                    cb(undefined);
                });
            });
